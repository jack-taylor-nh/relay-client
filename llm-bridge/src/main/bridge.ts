...
export class BridgeConnection {
  ...
  async connect(): Promise<void> {
    ...
    if (this.eventSource) {
      this.log('warn', 'Already connected', { edgeId: this.bridgeEdge.id });
      return;
    }
  ...
  private handleSSEMessage(event: MessageEvent): void {
    this.log('info', 'SSE message received', { data: event.data });
  }
  ...
  private async fetchMessage(messageId: string): Promise<any> {
    ...
    try {
      ...
      this.log('info', 'Message fetched successfully', { 
        ...
        hasEphemeralKey: !!data.ephemeralPubkey,
        hasNonce: !!data.nonce,
      });
  ...
  private async decryptMessage(message: any): Promise<{
    ...
    try {
      // Message contains Double Ratchet payload
      const { ciphertext, ephemeralPubkey, nonce } = message;
      
      if (!ciphertext || !ephemeralPubkey || !nonce) {
        throw new Error('Missing required encryption fields');
      }

      // The sender's edge public key should be in the message or loaded from conversation
      // For now, decrypt using our bridge's private key with the sender's ephemeral key
      const decrypted = crypto.decrypt(
        ciphertext,
        ephemeralPubkey,
        nonce,
        this.bridgeEdge.x25519PrivateKey
      );
      ...
      return {
        content: decrypted,
        senderEdgeId: message.edgeId, // Sender's edge ID from message
      };

    } catch (error) {
      this.log('error', 'Decryption error', error);
      return null;
    }
  }
  ...
  /**
   * Send encrypted response back to Relay server
   */
  private async sendResponse(
    ...
    try {
      ...
      // TODO: In full implementation, this should load from ratchet state or conversation participants
      // For now, we'll need to fetch it from the server or store it
      
      // Encrypt the response using Double Ratchet
      // Note: This is simplified - full implementation needs ratchet state management
      const { ciphertext, ephemeralPublicKey, nonce } = crypto.encrypt(
        content,
        recipientEdgeId // This should be recipient's X25519 public key
      );
      ...
      const response = await fetch(`${RELAY_API_BASE_URL}/v1/messages`, {
        ...
        body: JSON.stringify({
          ...
          signature: '', // TODO: Sign with bridge's private key
        }),
        signal: AbortSignal.timeout(RELAY_API_TIMEOUT),
      });
