...
export function EdgeCard({ 
  ...
  // Neutral color scheme - native handles use slate, email uses sky, discord uses indigo
  const bgColor = type === 'native' ? 'bg-slate-100' : type === 'discord' ? 'bg-indigo-100' : 'bg-sky-100';
  const textColor = type === 'native' ? 'text-slate-600' : type === 'discord' ? 'text-indigo-600' : 'text-sky-600';
  const badgeColor = type === 'native' ? 'bg-slate-600' : type === 'discord' ? 'bg-indigo-500' : 'bg-sky-500';
