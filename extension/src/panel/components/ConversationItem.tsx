...
import { Mail, Webhook, Link, AtSign, Bot, MoreVertical, Pencil, Archive, ArchiveRestore, Trash2 } from 'lucide-react';
import {
  DropdownMenu, DropdownMenuContent, DropdownMenuItem,
  DropdownMenuSeparator, DropdownMenuTrigger,
} from '@/components/ui/dropdown-menu';
import { conversationCustomNames } from '../state';
...
interface Props extends ConversationItemActions {
...
export function ConversationItem({
  ...
  const isLocalLlm = conversation.type === 'local-llm';
  const hasActions = !!(onRename || onArchive || onUnarchive || onDelete);
  const rawName =
    conversationCustomNames.value.get(conversation.id) ||
  ...
  return (
    <div
      className={cn(
        'group relative flex items-center w-full transition-colors duration-150',
        isSelected
          ? 'bg-[hsl(var(--accent))]'
          : isUnread
    ...
    >
      {/* Main clickable area */}
      <button
        className="flex items-center gap-3 flex-1 min-w-0 overflow-hidden px-4 py-3 text-left bg-transparent border-none cursor-pointer"
        onClick={onClick}
      >
        {/* Avatar icon */}
        <div
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="flex items-center gap-2 mb-0.5 min-w-0 overflow-hidden">
            <div className="flex items-center gap-1.5 flex-1 min-w-0 overflow-hidden">
              <span
                className={cn(
                  'min-w-0 shrink text-sm text-[hsl(var(--foreground))] whitespace-nowrap overflow-hidden text-ellipsis',
            ...
            {/* Time + security badge — fades on hover when actions exist */}
            <div
              className={cn(
                'flex items-center gap-1.5 flex-shrink-0 ml-auto transition-opacity duration-150',
        ...
        >
          <DropdownMenu onOpenChange={setMenuOpen}>
            <DropdownMenuTrigger asChild>
              <button
                className="p-1 rounded-md text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-[hsl(var(--accent))] bg-transparent border-none cursor-pointer"
            ...
            <DropdownMenuContent align="end" className="w-44">
              {onRename && (
                <DropdownMenuItem
                  onClick={(e: MouseEvent) => {
                    e.stopPropagation();
                    onRename(conversation.id, displayName);
                  }}
                >
              ...
              {onDelete && (onRename || onArchive || onUnarchive) && (
